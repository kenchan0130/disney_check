...
result=$(echo "${response}" | perl -ne '/ticketDayDetails = (.+?);/&&print"$1"' | jq '.[] | select(.date == "20201129")' | grep -E 'ticket_class' | grep -vE "(is-none|is-close)")

if [[ "${result}" ]];then
    curl -X POST --data-urlencode "payload={\"channel\": \"#notification\", \"username\": \"disney checker\", \"text\": \"チケットが見つかったよ https://www.tokyodisneyresort.jp/ticket/sales_status.html\", \"icon_emoji\": \":bot:\"}" "${SLACK_WEBHOOK_URL}"
fi
